...
## Dropped by enemies — see enemy.gd's money_drop_chance. Grants one coin on
## contact and is gone; duck-types the player the same way oxygen_pickup.gd
## and bomb_pickup.gd do, so this has no dependency on Player beyond the
## method existing.
